...
class StopWatch(Frame):
    ...
    def makeWidgets(self):
        ...
        l2.pack(fill=X,expand=NO,pady=3,padx=2)
        
        scrolllbar = Scrollbar(self,orient=VERTICAL)
        self.m = Listbox(self,selectmode=EXTENDED,height=5,yscrollcommand=scrolllbar.set)
        self.m.pack(side=LEFT,fill=BOTH,expand=1,pady=5,padx=2)
        scrolllbar.config(command=self.m.yview)
    ...
    def Lap(self):
        ...
        if self._runniing:
            self.laps.append(self._setLpaTime(tempo))
